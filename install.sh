...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="mac"; }
echo 'mac() { "$HOME/core-token/packages/core/bin/run" "$@" --token="mac"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/Highjhacker/my_awesome_chain_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Highjhacker/my_awesome_chain_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@mac"
rm -rf "$HOME/.config/@MyAwesomeChain"
rm -rf "$HOME/.config/mac-core"
